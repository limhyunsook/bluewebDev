...
trait kmpayFuncTrait{	
	...
	{
		//echo "[parameterEncrypt] key-----".$key."---text---".$plainText;
		try {
			$encryptText = "";
			$iv = "";
	...
	public function connMPayDLP($urlStr, $mid, $encryptStr) 
	{
		try {
			...
			$errno = curl_errno($ch);
			if ($errno != "0") $result = "_FAIL_,".$errno.",".$errcode;
	...
	//abstract public function getWorld();//강요
}

trait JsonStringTrait{
	private $LogPath = "";
...
{	
	...
	public function __construct($config)
    {
		...
		$this->LogDir = $LogDir = isset($config["LogDir"])? $config["LogDir"]: "";
		$this->LogPath = $LogPath = isset($config["LogPath"])? $config["LogPath"]: "";
		$this->phpVersion = $phpVersion = isset($config["phpVersion"])? $config["phpVersion"]: "";
	}
}
...
class JsonString 
{
	...
	public function __construct($config)
    {
		$LogDir = isset($config["LogDir"])? $config["LogDir"]: "";
	}
}

class KMPayDataValidator 
{
    use KMPayDataValidatorTrait;
	public function __construct($config)
    {
		$LogDir = isset($config["LogDir"])? $config["LogDir"]: "";
		$this->resultValid = $this->validator($config);		
	}
