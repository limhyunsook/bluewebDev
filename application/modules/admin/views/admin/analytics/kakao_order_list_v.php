		...
		<div class="content-inner">
			...
			<form action="" id="page_form" method="get" >
				...
				<select name="sfl" id="sfl" class="form-control">
					<option value="GoodsName" <? if(isset($input['sfl']) && $input['sfl']=="GoodsName") echo "selected";?>>상품명</option>
					<option value="PayMethod" <? if(isset($input['sfl']) && $input['sfl']=="PayMethod") echo "selected";?> >결제수단</option>
				</select>
				</div>
				<div class="col-md-9">
				<input type="text" name="stx" 		id="stx"  class="form-control"	value="<?=(isset($input['stx']) )? $input['stx'] : "" ?>" placeholder="상품명, 결제수단" />
				</div>
				...
				<tbody>
						...
						foreach ($lists as $key => $val): ?>
							<tr>
								...
								<td><?=$val['TID']?></td>
								<td><?=$val['GoodsName']?></td>
								<td><?=($val['BuyerName'])? $val['BuyerName'] : "비회원";?></td>
								<td><?=$val['PayMethod']?></td>
								<td><?=$val['CardCode']?></td>
								<td><?=($val['paySuccess'])? "결제성공":"결제실패";?></td>
								<td><?=$val['AuthDate']?></td>
							</tr>
						<? endforeach;  ?>
					</tbody>
