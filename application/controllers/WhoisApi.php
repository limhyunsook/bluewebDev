...
class WhoisApi extends MY_Controller
{
	 ...
	 * 
	 *  ?? 연산자는 PHP 7.0 이상에서 호환합니다. php 5.4 ~ 5.6 에서는 isset 으로 대체 전환 하십시오.
	 */
	...
	{
		parent::__construct();		
		if($this->input->get_post('json_type',true)) $this->json = $this->input->get_post('json_type',true);
	}
	
	/**
	 * json type 형태가 다른 값이 들어가면 패쓰
	 */
	public function exitjson($result)
	{
		if($this->json == 'json'){
	...
	{		
		...
		$request['arr_domain_name'] = $request['arr_domain_name']  ?? []; 	//testdoamin
		$request['arr_domain_type'] = $request['arr_domain_type']  ?? [];	//.com

		foreach($request as $key =>$val) $params["{$key}"] = $val;
		// $params["arr_domain_name"] = $request['arr_domain_name'];			// 조회할 도메인명 배열 (여러개 입력 가능)	: 필수
		// $params["arr_domain_type"] = $request['arr_domain_type'];			// 조회할 도메인 tld (여러개 입력 가능) : 필수
	...
	public function check()
	{
		...
		$whois = new Whois\WhoisDomainClient();	
		$api = $whois->getWhoisHeandler();
		$domain = $request['domain'] ?? "";	// 검색하려는 도메인명	checkdomain.com
	...
	public function host_info() 
	{
		...
		$request['arr_domain_name'] = 	$request['arr_domain_name']  ?? []; //testdomain0001
		$request['arr_host_name'] = 	$request['arr_host_name']  ?? []; //ns1

		foreach($request as $key =>$val) $params["{$key}"] = $val;
		// $params["arr_domain_name"] = 	$request['arr_domain_name'];				// 조회할 도메인명 배열 (여러개 입력 가능)	: 필수
		// $params["arr_host_name"] = 		$request['arr_host_name'];			// 조회할 호스트명 (여러개 입력 가능) : 필수		
	...
	public function host_manage() 
	{
		...
		$request['arr_domain_name'] = 		$request['arr_domain_name']  ?? []; //testdomain0001.com
		$request['arr_host_name'] = 	$request['arr_host_name']  ?? []; //ns1
		$request['arr_host_ip'] = 		$request['arr_host_ip']  ?? []; //58.151.236.81
		$request['operation'] = 	$request['operation']  ?? 'regist';


		// 파라미터
		$params = array();
		foreach($request as $key =>$val) $params["{$key}"] = $val;
		// $params["arr_domain_name"] = $request['arr_domain_name'];					// 도메인명 배열 (여러개 입력 가능)	: 필수
		// $params["arr_host_name"] = $request['arr_host_name'];						// 호스트명 (여러개 입력 가능) : 필수
		// $params["arr_host_ip"] = $request['arr_host_ip'];							// 아이피 (여러개 입력 가능) : 필수
		// $params["operation"] = $request['operation'];										// 등록 [regist], 수정 [modify], 삭제 [delete]
	...
	public function register() 
	{
		...
		foreach($this->input->get(NULL, TRUE) as $key => $val) $request["{$key}"]  = $val;
		foreach($this->input->post(NULL, TRUE) as $key => $val) $request["{$key}"]  = $val;
		$request['arr_domain_name'] = 		$request['arr_domain_name']  ?? [];	//testdomain00013
		$request['arr_domain_type'] = 		$request['arr_domain_type']  ?? []; //.com
		$request['arr_term'] = 				$request['arr_term']  ?? [1]; 
		$request['reg_name_loc'] = 			$request['reg_name_loc']  ?? ''; //홍길동
		$request['reg_name_eng'] = 			$request['reg_name_eng']  ?? ''; //GilDong,Hong
		$request['reg_email'] = 			$request['reg_email']  ?? ''; //testdomain0001
		$request['reg_zipcode'] = 			$request['reg_zipcode']  ?? ''; //12345
		$request['reg_address_loc1'] = 		$request['reg_address_loc1']  ?? ''; //서울시 구로구 구로3동
		$request['reg_address_loc2'] = 		$request['reg_address_loc2']  ?? ''; //대륭포스트타워3차 1101
		$request['reg_address_eng1'] = 		$request['reg_address_eng1']  ?? ''; //1101 Daeryung post tower
		$request['reg_address_eng2'] = 		$request['reg_address_eng2']  ?? ''; //guro3-dong gurogu seoul
		$request['reg_telephone'] = 		$request['reg_telephone']  ?? ''; //02-555-5555
		$request['reg_fax'] = 				$request['reg_fax']  ?? ''; //02-555-5555
		$request['reg_nation'] = 			$request['reg_nation']  ?? 'KR'; //KR
		$request['member'] = 				$request['member']  ?? '';
		$request['arr_host_name'] = 		$request['arr_host_name']  ?? []; //ns1.whoisdomain.kr
		$request['arr_host_ip'] = 			$request['arr_host_ip']  ?? [];  //211.206.125.156

		//params
		$params = array();		
		foreach($request as $key =>$val) $params["{$key}"] = $val;

		// $params["arr_domain_name"] = 		$request["arr_domain_name"];					// 등록할 도메인명 배열 (여러개 입력 가능)	: 필수
		// $params["arr_domain_type"] = 		$request["arr_domain_type"];					// 등록할 도메인 tld (여러개 입력 가능) : 필수
		// $params["arr_term"] 	= 			$request["arr_term"];								// 등록 기간 (여러개 입력 가능) : 필수
		// $params["reg_name_loc"] = 			$request["reg_name_loc"];							// 등록자명 한글 : 필수
		// $params["reg_name_eng"] = 			$request["reg_name_eng"];					// 등록자명 영문 : 필수
		// $params["reg_email"] = 				$request["reg_email"];				// 등록자 이메일 : 필수
		// $params["reg_zipcode"] = 			$request["reg_zipcode"];							// 등록자 우편번호 : 필수
		// $params["reg_address_loc1"] = 		$request["reg_address_loc1"];			// 등록자 한글주소1 : 필수
		// $params["reg_address_loc2"] = 		$request["reg_address_loc2"];			// 등록자 한글주소2 : 필수
		// $params["reg_address_eng1"] = 		$request["reg_address_eng1"];	// 등록자 영문주소1 : 필수
		// $params["reg_address_eng2"] =		$request["reg_address_eng2"];	// 등록자 영문주소2 : 필수
		// $params["reg_telephone"] =			$request["reg_telephone"];					// 전화번호 : 필수
		// $params["reg_fax"] = 				$request["reg_fax"];							// 팩스 : 옵션
		// $params["reg_nation"] = 			$request["reg_nation"];								// 국가 : 필수
		// $params["member"] = 				$request["reg_nation"];			// 회원 아이디 존재시 입력 : 옵션
		// $params["arr_host_name"] = 			$request["arr_host_name"];	// 네임서버 호스트명
		// $params["arr_host_ip"] = 			$request["arr_host_ip"];		// 네임서버 호스트명
	...
	public function renew()
	{
		...
		$request['arr_domain_name'] = 		$request['arr_domain_name']  ?? [];	//testdomain00013
		$request['arr_domain_type'] = 		$request['arr_domain_type']  ?? []; //.com
		$request['arr_term'] = 				$request['arr_term']  ?? [1]; //1
		$request['req_name'] = 				$request['req_name']  ?? ''; //홍길동
		$request['req_telephone'] = 		$request['req_telephone']  ?? ''; //010-111-1111
		$request['req_email'] = 			$request['req_email']  ?? ''; //trombon@whois.co.kr
		$request['member'] = 				$request['member']  ?? ''; //
		...
		$params["arr_domain_type"] = $request['arr_domain_type'];						// 등록할 도메인 tld (여러개 입력 가능) : 필수
		$params["arr_term"] = $request['arr_term'];								// 등록 기간 (여러개 입력 가능) : 필수		
		$params["req_name"] = $request['req_name'];							// 결제자 한글 : 필수
		$params["req_telephone"] = $request['req_telephone'];					// 결제자 전화번호 : 필수
		$params["req_email"] = $request['req_email'];				// 결제자 이메일 : 필수
		
		$params["member"] = $request['member'];				// 회원 아이디 존재시 입력 : 옵션		
	...
	public function renew_check() 
	{
		foreach($this->input->get(NULL, TRUE) as $key => $val) $request["{$key}"]  = $val;
		foreach($this->input->post(NULL, TRUE) as $key => $val) $request["{$key}"]  = $val;
		$request['domain'] = 		$request['domain']  ?? '';	//testdomain00013.com

		$params = array();
		$params["domain"] = $request['domain'];				// 조회할 도메인명 (배열아님)
	...
	public function transfer() 
	{
		foreach($this->input->get(NULL, TRUE) as $key => $val) $request["{$key}"]  = $val;
		foreach($this->input->post(NULL, TRUE) as $key => $val) $request["{$key}"]  = $val;
		$request['arr_domain_name'] = 		$request['arr_domain_name']  ?? [] ;	//gabia
		$request['arr_domain_type'] = 		$request['arr_domain_type']  ?? [] ;	//.com
		$request['arr_term'] = 				$request['arr_term']  ?? [1];	//1
		$request['arr_authInfo'] = 			$request['arr_authInfo']  ?? [];	//auth-password
		$request['req_name'] = 				$request['req_name']  ?? '';	//홍길동
		$request['req_telephone'] = 		$request['req_telephone']  ?? '';	//010-111-1111
		$request['req_email'] = 			$request['req_email']  ?? '';	//trombon@whois.co.kr
		$request['member'] = 				$request['member']  ?? '';	// 
	...
	public function update_info() 
	{
		...
		$request['arr_domain_name'] = 					$request['arr_domain_name']  ?? [] ;	//testdomain00013
		$request['arr_domain_type'] = 					$request['arr_domain_type']  ?? [] ;	//.com
		$request['arr_reg_name_loc'] = 					$request['arr_reg_name_loc']  ?? [] ;	//홍길동
		$request['arr_reg_name_eng'] = 					$request['arr_reg_name_eng']  ?? [] ;	//GilDong,Hong
		$request['arr_reg_email'] = 					$request['arr_reg_email']  ?? [] ;	//trombon@whois.co.kr
		$request['arr_reg_zipcode'] = 					$request['arr_reg_zipcode']  ?? [] ;	//123451
		$request['arr_reg_address_loc1'] = 				$request['arr_reg_address_loc1']  ?? [] ;	//서울시 구로구 구로3동1
		$request['arr_reg_address_loc2'] = 				$request['arr_reg_address_loc2']  ?? [] ;	//대륭포스트타워3차 11011
		$request['arr_reg_address_eng1'] = 				$request['arr_reg_address_eng1']  ?? [] ;	//1101 Daeryung post tower1
		$request['arr_reg_address_eng2'] = 				$request['arr_reg_address_eng2']  ?? [] ;	//guro3-dong gurogu seoul1
		$request['arr_reg_telephone'] = 				$request['arr_reg_telephone']  ?? [] ;	//02-555-55551
		$request['arr_reg_fax'] = 						$request['arr_reg_fax']  ?? [] ;	//02-111-12341
		$request['arr_reg_nation'] = 					$request['arr_reg_nation']  ?? ['KR'] ;	//KR
		$request['member'] = 							$request['member']  ?? '' ;	//
